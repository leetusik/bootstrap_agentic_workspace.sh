...
- **Default loop — plan → operator approves the readied plan → executor, repeated.** The orchestrator advances on its own: after each slice it **automatically moves on to plan the next** — it never stops to ask permission to start planning. For each slice, plan at the operator's gate (in Claude Code, plan mode): research read-only, surface any clarifying questions, then present the **readied plan** and wait for the operator's approval *of that plan*. The only pause is approving the finished plan — never a pause before planning begins. After approval, write the approved **native plan** to that slice's **own** `plan.md` (free-form — no template; let it incorporate any operator note), pulling context from `phase.md`; then dispatch the executor; then re-read state and head straight into planning the next slice. Never pre-fill another slice's `plan.md`. The operator's verbatim intent lives in the phase's `intent.md`, not duplicated per slice.
- **`auto` (operator opt-in) skips the plan approval.** If the operator invokes the skill with `auto` (e.g. `/do-whole-phase auto`; "run unattended" also counts), don't wait for approval of the readied plan: for each slice, plan → write `plan.md` → dispatch the executor → verify → finish → commit → next, to the end of the phase. `auto` waives only the plan-approval pause — the safety halts still apply: a `pending` slice/phase, or any `needs_operator` / `blocked` / failed executor return, STOPS the loop even in `auto`.
...
- Implementation and `fix` slices are implemented by the dispatched `slice-executor` subagent (Codex: inline, same procedure), not by the orchestrator directly. The orchestrator owns plan-mode planning, verification, `validate`, every status transition, and all commits; the executor writes `result.md` and the slice's `phase.md` notes but never commits or changes workflow state. `plan.md` is the orchestrator's free-form native plan, written at the slice's turn (just-in-time, no template). Decomposition stays in the orchestrator; review stays with `phase-reviewer`.
