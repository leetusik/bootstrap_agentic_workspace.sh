...
실행자는 slice의 위험도(`risk`)에 따라 세 티어 중 하나가 선택됩니다. 위험도 표시가 곧
비용 조절 장치입니다 — 기계적인 일은 싼 모델이, 어려운 일은 좋은 모델이 맡습니다.

| 티어 | 기본 모델 | 맡는 일 |
|---|---|---|
| `slice-executor-low` | Haiku | 판단이 필요 없는 기계적인 저위험 slice. 계획을 문자 그대로 따름 |
...
오케스트레이터가 발견 내용을 계획에 반영해 한 티어 위로 다시 맡깁니다. 항상 위로만
올라가고, slice당 최대 두 번입니다.
