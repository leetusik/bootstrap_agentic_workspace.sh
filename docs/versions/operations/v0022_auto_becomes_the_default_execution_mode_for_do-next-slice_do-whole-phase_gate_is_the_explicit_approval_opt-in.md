...
source: operator
summary: auto becomes the default execution mode for do-next-slice/do-whole-phase; gate is the explicit approval opt-in
previous: v0021_parallel_phase_execution_the_six_parallel-_commands_workspace_ci_the_parallel-phase_skill_and_the_deferred_consolidation_gate_v24
---
...
- **`Write` is the path wherever no harness plan file exists:** Codex (no plan mode)
  and `auto` (plan mode is never entered, so the harness writes no file). Since **v25**
  `auto` is the default, so `Write` is the **default** persistence path and the copy
  rule applies in the gated modes (`gate`, `plan only`).
- **Sites covered:** `do-next-slice` step 2 — its `gate` branch *and* its `plan only`
  branch, in both the `.claude/skills/` and `.agents/skills/` copies (bodies stay
  byte-identical) — plus `do-whole-phase`'s `gate` loop and its `plan only` branch
  (its default `auto` branch keeps `Write` explicitly), and both contract clauses in
