...
doc_id: operations
version: v0010
created_at: 2026-07-02T14:49:28+09:00
source: P5.REVIEW
summary: Installer: /explain is opt-in via --with-explain; --update preserves an installed copy
previous: v0009_installer_is_a_build_product_version-aware_update_flow_release_rule
---
...
and a viewer at `localhost:8765`), so since **v2** it is **opt-in / default-off**
at install time. The skill stays live in this repo and embedded in the built
artifact — the gate is a single install-time filter of the derived skill
inventories in `installer/main.py`, not a source removal.
...
- **Release note:** shipped as workspace **v2** (`WORKSPACE_VERSION = 2` in
  `installer/main.py` + the `## v2 — 2026-07-02` `CHANGELOG.md` entry, per the
  release rule below). Repos that installed `explain` under v1 keep it and keep
  receiving refreshes; migration notes: none.
