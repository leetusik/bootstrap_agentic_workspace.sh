...
doc_id: decisions
version: v0007
created_at: 2026-06-20T20:02:07+09:00
source: operator-request
summary: clarify do-whole-phase loop, add auto waiver, drop plan.md template
previous: v0006_engine_scaffolds_phase_intent.md_plus_a_create-phase_intake_skill_supersedes_convention-only_intent_persistence
---
...
- Decision:
  - **Name the default loop.** Both `do-next-slice` and `do-whole-phase` run, per slice, **plan (plan mode) → operator approves → executor**, repeated; the gate pauses before every slice.
  - **`auto` is the explicit waiver.** The operator invokes the skill with `auto` (e.g. `/do-whole-phase auto`; "run unattended" still counts) to skip the approval pauses and run plan → executor straight through. `auto` waives only the approval gate — the safety halts (`pending`, `needs_operator`, `blocked`, a failed/empty executor return) still STOP the loop. Applies to both skills.
