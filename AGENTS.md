...
- `rebuild` / `validate`
- `sync-agents [--check]` — apply the repo-root `.env` executor-tier config (models/efforts) to the six `slice-executor-*` agent files; run after editing `.env` and after a workspace update (`--check` reports drift without writing)
